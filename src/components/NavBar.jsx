...
import calblayIcon from "../assets/calblayIcon.png";
...
// Icona persona per al navbar
function IconPersona() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={t.textSecondary} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="8" r="4" />
      <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" />
    </svg>
  );
...
export default function Navbar({ nomAdmin = "nombre_admin" }) {
  const navigate = useNavigate();
  const [menuOpen, setMenuOpen] = useState(false);
  const menuRef = useRef(null);
  const isProduccion = true;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, paddingLeft: 20 }}>
        <span style={{ color: t.gold, fontSize: "0.8rem", fontWeight: 600, letterSpacing: "2px", cursor: "pointer" }}>
          EN ES
        </span>
      </div>
      ...
      {/* Right – Nav links + pill admin + icona persona + hamburguesa */}
      <div
        ref={menuRef}
        style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", position: "relative", gap: 12 }}
      >
        {/* Nav links */}
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <NavLink label="REGALA"     onClick={() => navigate("#")} />
          <span style={{ color: t.cardBorder }}>·</span>
          <NavLink label="AGENDA"     onClick={() => navigate("#")} />
          <span style={{ color: t.cardBorder }}>·</span>
          <NavLink label="CALÇOTADES" onClick={() => navigate("#")} />
          {isProduccion && (
            <>
              <span style={{ color: t.cardBorder }}>·</span>
              <NavLink label="CONTACTO" onClick={() => navigate("#")} />
            </>
          )}
        </div>
        ...
        >
          {menuOpen ? (
            ...
            <svg width="40" height="30" viewBox="0 0 18 14" fill="none">
